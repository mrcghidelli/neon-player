...
EXTERNAL_CAMERA_CONFIG = "frontal_caregiver"

class ExtCamera_Overlay_FrontalCaregiver(Observable, Plugin):
    icon_chr = chr(0xEC02)
    ...
    def __init__(
        ...
        try:
            with open(json_file_path, 'r') as f:
                ...
                self.mac_address = mac_data.get(EXTERNAL_CAMERA_CONFIG + "_mac_address", "b0-4a-6a-e9-80-69")
        except (FileNotFoundError, json.JSONDecodeError) as e:
            print(f"Error loading MAC addresses: {e}")
            self.mac_address = "b0-4a-6a-e9-80-69"  # Default fallback
