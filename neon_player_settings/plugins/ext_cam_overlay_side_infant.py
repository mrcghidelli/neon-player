...
EXTERNAL_CAMERA_CONFIG = "side_infant"

class ExtCamera_Overlay_SideInfant(Observable, Plugin):
    icon_chr = chr(0xEC02)
    ...
    def __init__(
        ...
        try:
            with open(json_file_path, 'r') as f:
                ...
                self.mac_address = mac_data.get(EXTERNAL_CAMERA_CONFIG + "_mac_address", "b0-4a-6a-e9-80-7d")
        except (FileNotFoundError, json.JSONDecodeError) as e:
            print(f"Error loading MAC addresses: {e}")
            self.mac_address = "b0-4a-6a-e9-80-7d"  # Default fallback
